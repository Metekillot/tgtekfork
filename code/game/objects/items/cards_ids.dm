...
/obj/item/card/id/advanced/chameleon
	...
	var/datum/weakref/chameleon_forge
...
/obj/item/card/id/advanced/chameleon/attack_self(mob/user)
	if(!user.can_perform_action(user, NEED_DEXTERITY| FORBID_TELEKINESIS_REACH))
		return ..()
	var/popup_input = tgui_input_list(user, "Choose Action", "Agent ID", list("Show", "Forge", "Change Account ID"))
	if(!popup_input || !after_input_check(user))
		return TRUE
	switch(popup_input)
		...
		if("Show")
			return ..()
		if("Forge")
			if(chameleon_forge && chameleon_forge.resolve())
				astype(chameleon_forge.resolve(), /datum/chameleon_card_forge)?.ui_interact(user)
				return ..()
			chameleon_forge = WEAKREF(new /datum/chameleon_card_forge(user, src))
			return ..()
